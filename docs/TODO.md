...
Last update: 2026-08-31 (TODO 5b: embed metadata while processing).
...
- [ ] 5b. While processing, embed **available** metadata into each owned `.mp3` / `.wav` / `.m4a`: artist, album, title, genre, duration, BPM, key, comments, cue points — anything the lookup has. Python module in the processing pipeline (`crate/` or `~/local_tools`), talking to the same class of store databases OneTagger uses (Beatport / Traxsource / Discogs). Fill empties; do not overwrite ReCK/MiK key, BPM, comments, or cues when those are already set. Never mutagen-write `.stem.m4a` (strips the NI stem atom; Traktor loses four decks). For STEMs, write the same metadata as a **JSON sidecar in parallel with the tree** — sibling `{name}.stem.json` next to `{name}.stem.m4a`, same folders, not in git. Never `.m4p`. Music.app still needs AppleScript after file writes (4a). Dry-run first. Detail: `docs/notes.md`.
