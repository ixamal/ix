...
## Crate + hardware
Unknown Album dump **executed**. Inbox catalog + Ollama (`qwen2.5:7b`) **executed**. Traktor NML remapped. DJCU2 Traktor → Rekordbox **worked**. Path-stable crate **confirmed** (TODO 3). Accapella + `EDM, …` genre **closed** (TODO 4: file tags + Music.app AppleScript; leftover `EDM*` 0). Sync Library Off. Next on Mac: `git pull`, then review `docs/TODO.md`. First numbered open item is still **5** (empty genre when Beatport v4 works). Parked: **5b** embed available metadata while processing (Python module, OneTagger-class stores; never mutagen-save `.stem.m4a`; STEM metadata as sibling `{name}.stem.json` in the same tree). Then Elysium. NI/Maschine idea **unblocked** (crate stable) but still parked — 16ch then A/B/C → Maschine → S88; do not wire until asked. Detail: `docs/notes.md`. Rig: [ixamal/blackhole](https://github.com/ixamal/blackhole). Bridge: [ATGR DJCU2](https://atgr.nl/). Snapshots: `databases/`. Process: `docs/djcu2.md`. Step log: `docs/onetagger.md`.
